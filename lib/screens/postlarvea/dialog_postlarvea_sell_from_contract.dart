...
class DialogPostlarveaSellFromContract extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    //Data data = InheritedProvider.of()
    // TODO: implement createState
    return new DialogPostlarveaSellFromContractState();
  }
}
class DialogPostlarveaSellFromContractState extends State<DialogPostlarveaSellFromContract> {
  ...
  List<String> getData;
  String idCon, stage,idStaff, idHatchery;
  int curNum = -1;
  ...
  Future<Data> _getInfoOfPostlarveaContract(String idCon) async{
    return memCache.runOnce(() async {
      http.Response response = await http.post(
          ...
          body: utf8.encode(Contract.queryGetInfoOfPostlarveaContractBuy(idCon)),
  ...
  _postlarveaSellFromContract(BuildContext context) async {
    if(txtNum.text.toString().length != 0 &&  curIDBuyer != "" &&
      ...
      if(num <= curNum && curNum > 0) {
        http.Response response = await http.post(
            ...
            body: utf8.encode(Postlarvea.querySellPostlarveaFromContract(
                idCon, num, idHatchery, idHatReceiver, signDate, note, stage, idFarmer, idStaff, expectedDate, bonus)),
            encoding: Encoding.getByName("UTF-8")
        );
        print(response.body);
        var p = response.body.indexOf("[");
  ...
  Widget build(BuildContext context) {
    ...
    return new Scaffold(
      ...
      body: SingleChildScrollView(
          child: FutureBuilder(
              future: _getInfoOfPostlarveaContract(idCon),
              builder: (BuildContext context, AsyncSnapshot snapshot) {
                if (snapshot.hasError) {
                  ...
                  curNum = int.parse(postlarvea.att5.toString());
                  var dateAssign = DateTime.parse('${defaultDate.format(defaultCSDL.parse(postlarvea.att2))}');
                  var dateNow = DateTime.now();
                  var difference = dateNow.difference(dateAssign).inDays + int.parse(postlarvea.att12.toString());
                  ...
                  return Column(
                    children: <Widget>[
                      ...
                      ViewsWidget.buildRowDetails(context,'number_of_postlarvea', '${function.formatNumber(curNum.toString())}'),
                      Padding(
                          ...
                          Container(
                            ...
                            child: Material(
                              ...
                              child: MaterialButton(
                                ...
                                onPressed: (){
                                  _postlarveaSellFromContract(context);
                                },

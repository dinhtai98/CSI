...
import 'package:flutter_helloworld/screens/postlarvea/postlarvea_get_meta_history_from_contract.dart';
...
import 'list_of_postlarvea_sell_from_contract.dart';
import 'package:postgres/postgres.dart';
import '../../Data/ConvertQueryResult.dart';

class PostlarveaContractBuy extends StatefulWidget {

  @override
  State<StatefulWidget> createState() {
    //Data data = InheritedProvider.of()
    // TODO: implement createState
    return new PostlarveaContractBuyState();
  }
}
class PostlarveaContractBuyState extends State<PostlarveaContractBuy> {

  String idCon, idTransport, idSeller, idBuyer, idRanking;

  AsyncMemoizer<Data> memCache = AsyncMemoizer();
  Future<Data> _getInfoPostlarveaContractBuy(String idCon) async{
  ...
  Widget build(BuildContext context) {
    ...
    return new Scaffold(
      ...
      body: SingleChildScrollView(
          child: FutureBuilder(
              future: _getInfoPostlarveaContractBuy(idCon),
                      ...
                      Card(
                        ...
                        child: Column(
                          children: <Widget>[
                            ...
                            ViewsWidget.buildRowDetails(context,'stage_of_postlarvea', '${contract.att12}'),
                            ViewsWidget.buildRowDetails(context, 'number_of_postlarvea', '${function.formatNumber(contract.att4)}'),
  ...
  _onTapHistory(BuildContext context){
    Navigator.of(context).push(new MaterialPageRoute<Null>(
        builder: (_) =>
            InheritedProvider<String>(
              child: new PostlarveaMetaHistoryFromContract(),
              inheritedData: idCon,
            ))).then((onValue) {
      setState(() {
        memCache = AsyncMemoizer();
  ...
  _onTapSell(BuildContext context){
    Navigator.of(context).push(new MaterialPageRoute<Null>(
        builder: (_) =>
            InheritedProvider<String>(
              child: new ListOfPostlarveaSellFromContract(),
              inheritedData: idCon,
            ))).then((onValue) {
      setState(() {
        memCache = AsyncMemoizer();
